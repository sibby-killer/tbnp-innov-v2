...
try {
    ...
    if (!empty($search)) {
        $where_conditions[] = "(o.id LIKE ? OR c.company_name LIKE ? OR o.pickup_location LIKE ? OR o.delivery_location LIKE ? OR t.plate_number LIKE ? OR o.tracking_number LIKE ? OR o.sender_name LIKE ? OR o.recipient_name LIKE ?)";
        $search_term = "%$search%";
        $params[] = $search_term;
        $params[] = $search_term;
        $params[] = $search_term;
    ...
    // Get orders with pagination - USING YOUR ACTUAL COLUMN NAMES
    $order_sql = "
        SELECT 
            o.*,
            c.company_name,
            ...
            DATE_FORMAT(o.estimated_pickup, '%d %b %Y %H:%i') as pickup_time_formatted,
            DATE_FORMAT(o.estimated_delivery, '%d %b %Y %H:%i') as delivery_time_formatted,
            DATEDIFF(o.estimated_delivery, NOW()) as days_remaining,
            o.weight,
            o.dimensions,
            o.service_type as priority,
            o.total_amount as shipping_cost
        FROM orders o
        LEFT JOIN clients c ON o.client_id = c.id
        ...
        LEFT JOIN couriers cou ON o.courier_id = cou.id
        $where_clause
        ORDER BY 
            CASE 
                WHEN o.service_type = 'express' THEN 1
                WHEN o.service_type = 'overnight' THEN 2
                ELSE 3 
            END,
...
try {
    ...
    // Pending orders (assuming status_id 1-3 are pending)
    ...
    // Delivered orders (assuming status_id 6 is delivered)
    ...
    // Total revenue
    $revenue_stmt = $db->prepare("SELECT SUM(total_amount) as total FROM orders WHERE status_id = 6");
...
/* Priority badges - based on service_type */
.priority-express {
            ...
            <form method="GET" action="" class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control" name="search" 
                           ...
                           placeholder="Order ID, Tracking, Client, Address...">
                </div>
                            ...
                            <?php foreach ($orders as $order): ?>
                                <tr>
                                    ...
                                    <td>
                                        <div class="small">
                                            <div class="text-truncate" style="max-width: 150px;" 
                                                 title="<?php echo htmlspecialchars($order['sender_name']); ?> (<?php echo htmlspecialchars($order['sender_phone']); ?>)">
                                                <i class="fas fa-user text-primary me-1"></i>
                                                <?php echo htmlspecialchars($order['sender_name']); ?>
                                            </div>
                                            <div class="text-truncate mt-1" style="max-width: 150px;" 
                                                 title="<?php echo htmlspecialchars($order['recipient_name']); ?> (<?php echo htmlspecialchars($order['recipient_phone']); ?>)">
                                    ...
                                    <td>
                                        <div class="small">
                                            <div class="text-truncate" style="max-width: 150px;" 
                                                 title="<?php echo htmlspecialchars($order['pickup_location']); ?>">
                                                <i class="fas fa-arrow-up text-success me-1"></i>
                                                <?php echo htmlspecialchars($order['pickup_location']); ?>
                                            </div>
                                            <div class="text-truncate mt-1" style="max-width: 150px;" 
                                                 title="<?php echo htmlspecialchars($order['delivery_location']); ?>">
                                    ...
                                    <td>
                                        <?php if ($order['priority'] == 'express'): ?>
                                            <span class="badge priority-express service-badge">Express</span>
                                        <?php elseif ($order['priority'] == 'overnight'): ?>
                                            <span class="badge priority-overnight service-badge">Overnight</span>
                                        <?php else: ?>
                                            <span class="badge priority-standard service-badge">Standard</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <div class="fw-bold">KSh <?php echo number_format($order['total_amount'], 2); ?></div>
                                        <?php if ($order['weight']): ?>
                ...
                <tbody>
                    <?php foreach ($orders as $order): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($order['sender_name']); ?> → <?php echo htmlspecialchars($order['recipient_name']); ?></td>
                            <td>
